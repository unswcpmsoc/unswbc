...
# Hard code a bunch of loookups since this is a hot path
...
class Position:
    ...
    def add_dir(self, direction: Direction, game_state: "Game" = None) -> "Position":
        active_game = game_state if game_state is not None else game
...
class Vision:
    ...
    def _tile(self, i: int) -> "Tile":
        ...
        if tile is None:
            ...
            if entity is None and has_pearl == "1":
                entity = Pearl(position)
            # Horizontal edges come VISION_SIZE to a row, so the one under a tile is a row
            # further on. Vertical edges come VISION_SIZE + 1 to a row, west side first.
            west = i + i // Constants.VISION_SIZE
            horizontal, vertical = self._horizontal_edges, self._vertical_edges
            tile = self._tiles[i] = Tile(position, entity, int(pearl_time), (
...
class Tile:
    __slots__ = ("position", "entity", "pearl_time", "_edges", "_edges_by_direction")

    def __init__(self, position: Position, entity: "Entity", pearl_time: int,
...
class DragonPart(Entity):
    __slots__ = ("dragon_id", "team", "dir", "is_dragon_head")
...
class Edge:
    ...
    def __init__(self, is_horizontal: bool, edge_type: EdgeType, portal_id: int = None):
...
class Controller:
    ...
    def __init__(self, dragon_id: int, team: Team, direction: Direction,
        ...
        self.unit_count = 1
        self.unit_limit = unit_limit
        # We don't get the position until the first round
    ...
    def make_moves(self, directions: list[Direction]) -> bool:
        if (not directions or len(directions) > self.length - 1
                or any(not isinstance(direction, Direction) for direction in directions)):
            return False
    ...
    def do_split(self, child_size: int) -> bool:
        if not self.can_split(child_size):
            return False
    ...
    # def set_global_buffer(self, index: int, value: int) -> bool:
    #     if (index < 0 or index >= Constants.BUFFER_SIZE
    #             or not _is_uint32(value)):
    #         return False
    #     self.buffer[index] = value
    #     print(f"SETBUF {index} {value}", flush=True)
    #     return True

    # def read_global_buffer(self, index: int) -> int:
    #     if index < 0 or index >= Constants.BUFFER_SIZE:
    #         raise IndexError(f"buffer index must be between 0 and {Constants.BUFFER_SIZE - 1}")
...
# stdin is written by the engine (BuildInitBlock and BuildRoundBlock in
# engine/src/protocol.cc), so labels and values are not checked.
...
def update(controller: Controller, game_state: Game) -> bool:
    ...
    # the judge ends each round's input with a blank line
    first = read()
    while first == "\n":
        first = read()
    if first == "":
    ...
    # The rest of the round is kept as text. Vision parses it when the bot asks for tiles.
...
def end_turn():
    # Every write is charged 2.5 million points, so the turn's output goes out
    # in one go here.
